...
#pragma omp parallel for default(none) shared(                                 \
  ...
  void hap_sim_rand(int step_w, int step_h) {
    // int step_w = 1000;
    // int step_h = 100;
    // TODO results are uncorrect due to PA/DA definition
    for (unsigned int col = 0; col < this->width; col += step_w) {
      // std::cerr << "\n" << col << ": ";
      ...
      int j = 0;
      int u_min_h = hap;
      int lcp = this->phi->plcp(hap, col);
      int u_min_v = lcp;
      auto start_row = hap;
      int u = 0;
      auto next = this->phi->phi_inv(hap, col);
      if (next.has_value()) {

        next = this->phi->phi_inv(next.value(), col);
        if (next.has_value())
          lcp = this->phi->plcp(next.value(), col);
        while (next.has_value() && j != step_h && lcp > 0) {
          if (lcp < u_min_v) {
            u++;
            u_min_v = lcp;
            u_min_h = next.value();
      ...
      j = 0;
      int d_min_h = hap;
      lcp = this->phi->plcp(hap, col);
      int d_min_v = lcp;
      start_row = hap;

      next = this->phi->phi(hap, col);
      while (next.has_value() && j != step_h && lcp > 0) {
        if (lcp < d_min_v) {
          u++;
          d_min_v = lcp;
        ...
        next = this->phi->phi(next.value(), col);
        j++;
        if (next.has_value())
          lcp = this->phi->plcp(next.value(), col);
      }

      // std::cout << j << " [" << hap << ", " << col << "]: (" << u_min_h
      //           << ", " << u_min_v << ", " << u << "), (" << d_min_h << ",
      //           "
      //           << d_min_v << ", " << u << ")\n";
    }
  ...
  void hap_sim(int step_w, int step_h) {
    // int step_w = 1000;
    // int step_h = 100;
    // int start = 0;
    // TODO results are uncorrect due to PA/DA definition
    for (unsigned int col = 0; col < this->width; col += step_w) {
      // std::cerr << "\n" << col << ": ";
      for (unsigned int hap = 0; hap < this->height; ++hap) {
        std::cerr << col << ": " << hap << "\r";
        // std::random_device dev;
        // std::mt19937 rng(dev());
        // std::uniform_int_distribution<std::mt19937::result_type> dist(
