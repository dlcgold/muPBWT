...
private:
  ...
  maxSubvector(const std::vector<unsigned int> &vector, int k) {
    ...
    int max_min_value = std::numeric_limits<int>::min();
    int max_min_index = -1;
    std::deque<int> window_min_values;

    for (int i = 0; i < vector.size(); ++i) {
      // Remove elements outside the current window
      while (!window_min_values.empty() &&
             window_min_values.front() < i - k + 1) {
        window_min_values.pop_front();
      }

      // Remove elements in the current window that are greater than the current
      // element
      while (!window_min_values.empty() &&
             vector[window_min_values.back()] > vector[i]) {
        window_min_values.pop_back();
      }

      // Add the current element index to the window
      window_min_values.push_back(i);

      // Check if the window is of size k
      if (i >= k - 1) {
        // The front of the deque contains the index of the minimum element in
        // the current window
        int min_value = vector[window_min_values.front()];

        // Update if the current minimum is larger than the previous maximum
        // minimum
        if (min_value > max_min_value) {
          max_min_value = min_value;
          max_min_index = window_min_values.front();
        }
      }
    }
  ...
  void update_k_intervals(const std::vector<unsigned int> &div,
    ...
    // }
    // std::cout << "\n";
    // std::cout << "col before " << this->cols.back().p << "\n";
    for (unsigned int i = 0; i < this->cols.back().p.size(); i++) {
      // for (unsigned int j = 0; j < div.size(); j++) {
      //   std::cout << div[j] << " ";
      // }
      // std::cout << "\n";
      // auto pos = this->cols.back().p[i];
      auto lf_pos =
      ...
      auto a = 0;
      if (lf_pos + 1 >= this->k_smem) {
        a = lf_pos - this->k_smem + 1;
      }
      auto e = h - 1;
      ...
      // lf_pos = rlpbwt_int::lf(this->cols.size() - 1, pos,
      //                         get_next_char(this->cols.back().zero_first,
      //                         pos));
      ...
      // std::cout << "lf_pos " << lf_pos << " a " << a << " e " << e << "
      // sub_m
      // "
      //           << sub_m.first << " div_m " << div_m << " off " << s <<
      //           "\n";
      //  std::cout << a + off << " " << a + off + k_smem - 1 << "\n";
    }
    // std::cout << "------------------\n";
    //  std::cout << "col after " << this->cols.back().p << "\n";
...
public:
  ...
  compute_ms_k(const std::string &query, bool extend_matches = false,
    ...
    auto s_index =
        curr_index - this->cols[0].i_k[this->cols[0].sample_end.size() - 1];
        ...
        if (query[i] != symbol) {
          ...
          if (this->cols[i].sample_beg.size() == 1) {
            ...
            if (i != query.size() - 1) {
              ...
              s_index = curr_index -
                        this->cols[i + 1]
                            .i_k[this->cols[i + 1].sample_end.size() - 1];
              curr_run = index_to_run(curr_index, i + 1);
              symbol = get_next_char(this->cols[i + 1].zero_first, curr_run);
              if (verbose) {
                std::cout << "update: " << curr_index << " " << curr_pos << " "
          ...
          if ((curr_index == this->height - 1) ||
            ...
            ms.row[i] = curr_pos;
            ms.row_supp[i] = this->cols[i].i_e_k[curr_run];
            // ms.row_supp[i] = s_index;
            ms_supp[i] = curr_index;
            ...
            if (i != query.size() - 1) {
              ...
              s_index = curr_index - this->cols[i].i_k[curr_run];
              curr_run = index_to_run(curr_index, i + 1);
              symbol = get_next_char(this->cols[i + 1].zero_first, curr_run);
              if (verbose) {
                std::cout << "new: " << curr_index << " " << s_index << " "
            ...
            unsigned int r_b = index_to_run(b, i);
            unsigned int r_e_b = index_to_run(b + this->k_smem, i);
            if (r_b == this->cols[i].p.size() - 1) {
              b = this->cols[i].sample_end[this->cols[i].sample_end.size() - 1];
              b_i = this->height - 1;
            } else {
              auto min_b = 0;
              auto len_min_b = this->width;
              for (unsigned int r = r_b; r < r_e_b; r++) {
                auto tmp_i = 0;
                if (r == this->cols[i].p.size() - 1) {
                  tmp_i = this->height - 1;
                } else {
                  tmp_i = this->cols[i].p[r + 1] - 1;
                }
                if (get_next_char(this->cols[i].zero_first,
                                  index_to_run(tmp_i, i)) == query[i]) {
                  b = this->cols[i].sample_end[index_to_run(tmp_i, i)];
                  b_i = this->cols[i].p[index_to_run(tmp_i, i) + 1] - 1;
                  break;
                }
              }
            }
