...
rl_column::rl_column(bool zeroFirst, unsigned int count0, sdsl::int_vector<> p,
    ...
    : zero_first(zeroFirst), count_0(count0), p(std::move(p)),
      uv(std::move(uv)), t(std::move(t)), sample_beg(std::move(sample_beg)),
      sample_end(std::move(sample_end)),
      sample_beg_lcp(std::move(sample_beg_lcp)) {
  this->i_k = sdsl::int_vector<>(this->p.size(), 0);
  this->l_k = sdsl::int_vector<>(this->p.size(), 0);
  this->i_e_k = sdsl::int_vector<>(this->p.size(), 0);
  this->l_e_k = sdsl::int_vector<>(this->p.size(), 0);
}
...
std::ostream &operator<<(std::ostream &os, const rl_column &naive) {
  ...
  if (naive.t.empty()) {
    for (unsigned int i = 0; i < naive.p.size(); i++) {
      os << naive.p[i] << "\t" << naive.uv[i] << "\t(" << naive.sample_beg[i]
         << ", " << naive.sample_end[i] << ")\n";
    }
  } else {
    for (unsigned int i = 0; i < naive.p.size(); i++) {
      os << naive.p[i] << "\t" << naive.uv[i] << "\t" << naive.t[i] << "\t("
         ...
         << "\t(" << naive.l_k[i] << ", " << naive.i_k[i] << ")\t"
         << "\t(" << naive.l_e_k[i] << ", " << naive.i_e_k[i] << ")\t" << value
         << "\n";
      if (value[0] == '0') {
        value[0] = '1';
      } else {
...
unsigned long long rl_column::size_in_bytes(bool verbose) const {
  ...
  auto k_size =
      sdsl::size_in_bytes(this->i_k) + sdsl::size_in_bytes(this->l_k) +
      sdsl::size_in_bytes(this->i_e_k) + sdsl::size_in_bytes(this->l_e_k);
  size += p_size;
  size += uv_size;
  size += t_size;
  size += sample_size;
...
double rl_column::size_in_mega_bytes(bool verbose) const {
  ...
  auto k_size = sdsl::size_in_mega_bytes(this->i_k) +
                sdsl::size_in_mega_bytes(this->l_k) +
                sdsl::size_in_mega_bytes(this->i_e_k) +
                sdsl::size_in_mega_bytes(this->l_e_k);
  size += p_size;
  size += uv_size;
  size += t_size;
  size += sample_size;
...
size_t rl_column::serialize(std::ostream &out, sdsl::structure_tree_node *v,
  ...
  this->i_k.serialize(out, child, "i_k");
  this->l_k.serialize(out, child, "l_k");
  this->i_e_k.serialize(out, child, "i_e_k");
  this->l_e_k.serialize(out, child, "l_e_k");
