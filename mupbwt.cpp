...
void printHelp() {
  ...
  std::cout << "  -k, --ksmem\t compute k-smems" << std::endl;
...
int main(int argc, char **argv) {
  ...
  while (true) {
    static struct option long_options[] = {
        ...
        {"ksmem", required_argument, nullptr, 'k'},
        {"verbose", no_argument, nullptr, 'v'},
        {"help", no_argument, nullptr, 'h'},
        {nullptr, 0, nullptr, 0}};

    int option_index = 0;
    c = getopt_long(argc, argv, "i:s:l:o:q:emvdk:h", long_options,
                    &option_index);
  ...
  if (load_file.empty()) {
    ...
    rlpbwt_int rlpbwt(matrix_input.c_str(), verbose, macs, k_smem);
    // auto j = 0;
    //  for (auto c : rlpbwt.cols) {
    //    std::cout << j << "\n";
    //    for (unsigned int i = 0; i < c.p.size(); i++) {
    //      std::cout << i << " " << c.p[i] << " " << c.i_k[i] << " " <<
    //      c.e_k[i]
    //                << "\n";
    //    }
    //    j++;
    ...
    if (query) {
      if (k_smem <= 1) {
        ...
        std::cout << "start querying " << k_smem << "-smem\n";
      }
      START = clock();
      rlpbwt.query_match(query_input.c_str(), output.c_str(), verbose, macs);
